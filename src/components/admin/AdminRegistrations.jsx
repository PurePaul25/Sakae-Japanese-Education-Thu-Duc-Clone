...
const StatusBadge = ({ status }) => {
    ...
    return (
        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-semibold whitespace-nowrap ${meta.cls}`}>
            {meta.label}
        </span>
    );
};
...
const DeleteConfirmModal = ({ registration, onConfirm, onCancel, loading, show, duration = 300 }) => (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-5">
        <div className={`absolute inset-0 bg-black/70 transition-opacity duration-${duration} ${show ? 'opacity-100' : 'opacity-0'}`} />
        <div className={`relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl p-6 transform transition-all duration-${duration} ${show ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'}`}>
            <div className="flex flex-col items-center text-center gap-4">
                ...
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">
                        ...
                        <span className="font-semibold text-slate-700 dark:text-slate-200">{registration?.fullName}</span>?
                        Hành động này không thể hoàn tác.
                    </p>
                </div>
                <div className="flex gap-3 w-full">
                    <button onClick={onCancel} className="flex-1 px-4 py-2.5 cursor-pointer border border-slate-200 dark:border-slate-700 rounded-xl font-semibold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors">Huỷ</button>
                    <button onClick={onConfirm} disabled={loading} className="flex-1 px-4 py-2.5 cursor-pointer bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold transition-colors disabled:opacity-60">
...
const DetailModal = ({ registration: reg, onClose, onStatusChange, updatingId, show, duration = 300 }) => {
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 sm:p-6">
            <div className={`absolute inset-0 bg-black/70 transition-opacity duration-${duration} ${show ? 'opacity-100' : 'opacity-0'}`} onClick={onClose} />
            <div className={`relative bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto transform transition-all duration-${duration} ${show ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'}`}>
                <div className="flex items-center justify-between p-5 border-b border-slate-300 dark:border-slate-800 sticky top-0 bg-white dark:bg-slate-900 z-10">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-slate-800 dark:text-white leading-tight">Chi tiết đăng ký</h3>
                            <p className="text-sm text-slate-400">#{reg.id?.slice(0, 8)}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="p-2 cursor-pointer rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 transition-colors"><X size={18} /></button>
                </div>
                <div className="p-5 grid gap-5">
                    <section className="grid gap-3">
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Thông tin học viên</h4>
                        ...
                        {reg.note && (
                            <div className="bg-slate-100 dark:bg-slate-800/50 rounded-xl p-3 flex gap-2">
                                ...
                                <div>
                                    ...
                                    <p className="text-base text-slate-700 dark:text-slate-300 whitespace-pre-wrap">{reg.note}</p>
                                </div>
                            </div>
                        )}
                    </section>
                    ...
                    {schedule && (
                        <section className="grid gap-3">
                            ...
                            <div className="grid sm:grid-cols-2 gap-3">
                                <InfoRow icon={<Calendar size={16} />} label="Ngày khai giảng" value={fmt(schedule.startDate)} />
                                <InfoRow icon={<Clock size={16} />}    label="Giờ học"         value={schedule.time || '—'} />
                                <InfoRow icon={<Calendar size={16} />} label="Ngày học"        value={schedule.studyDays || '—'} />
                                {schedule.teacher && <InfoRow icon={<User size={16} />} label="Giáo viên" value={schedule.teacher} />}
                            </div>
                        </section>
                    )}
                    <section className="grid gap-3">
                        <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Thời gian & Trạng thái</h4>
                        <div className="grid sm:grid-cols-2 gap-3 items-center">
                            <InfoRow icon={<Calendar size={16} />} label="Ngày đăng ký" value={fmtDatetime(reg.createdAt)} />
                            <div className="flex items-center gap-2">
                                ...
                                <div className="relative flex items-center">
                                    <DropdownSelect
                                        ...
                                        options={ALL_STATUSES.map((status) => ({ label: STATUS_META[status].label, value: status }))}
                                        placeholder="Chọn trạng thái"
                                        buttonClassName="text-sm"
                                        disabled={updatingId === reg.id}
                                    />
                                    {updatingId === reg.id && <Loader2 size={12} className="absolute right-1.5 text-red-500 animate-spin" />}
                                </div>
                            </div>
                        </div>
                    </section>
...
const AdminRegistrations = () => {
    ...
    useEffect(() => { setPage(1); }, [statusFilter, courseFilter, sort]);
    ...
    const fetchRegistrations = useCallback(async (p = 1, isRefresh = false) => {
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            ...
            fetchRegistrations(page, true);
            fetchStats();
        } catch (err) { addToastRef.current(err?.response?.data?.message || 'Xóa thất bại.', 'error'); }
        finally { setDeleting(false); }
    };

    const openDetail = (reg) => { setDetailTarget(reg); setTimeout(() => setShowDetailModal(true), 10); };
    const closeDetail = () => { setShowDetailModal(false); setTimeout(() => setDetailTarget(null), MODAL_DURATION); };
    const openDelete  = (reg) => { setDeleteTarget(reg);  setTimeout(() => setShowDeleteModal(true), 10); };
    const closeDelete = () => { setShowDeleteModal(false); setTimeout(() => setDeleteTarget(null), MODAL_DURATION); };
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
                <StatCard label="Tổng đăng ký"  value={stats?.totalRegistrations} colorCls="text-slate-800 dark:text-white"        loading={statsLoading} />
                <StatCard label="Chờ liên hệ"   value={stats?.pending}            colorCls="text-amber-600 dark:text-amber-400"    loading={statsLoading} />
                <StatCard label="Đã liên hệ"    value={stats?.contacted}          colorCls="text-blue-600 dark:text-blue-400"      loading={statsLoading} />
                <StatCard label="Đã xác nhận"   value={stats?.confirmed}          colorCls="text-green-600 dark:text-green-400"    loading={statsLoading} />
                <StatCard label="Đã hủy"        value={stats?.cancelled}          colorCls="text-slate-500 dark:text-slate-400"    loading={statsLoading} />
            </div>
                    ...
                    <table className="w-full text-left min-w-[800px]">
                        <thead>
                            <tr className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                                {['Họ tên / Email', 'Điện thoại', 'Khóa học', 'Lịch học', 'Trạng thái', 'Ngày ĐK', ''].map((h, i) => (
                                    <th key={i} className="px-4 py-3 text-[12px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider whitespace-nowrap">{h}</th>
                                ))}
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {loading ? <TableSpinner />
                            : registrations.length === 0 ? <EmptyState />
                            : registrations.map((reg) => {
                                ...
                                return (
                                    <tr key={reg.id} className="hover:bg-slate-100/80 dark:hover:bg-slate-800/40 transition-colors">
                                        ...
                                        <td className="px-4 py-2.5">
                                            <p className="text-sm text-slate-600 dark:text-slate-300">{reg.phone}</p>
                                            {reg.zalo && <p className="text-xs text-slate-400 mt-0.5">Zalo: {reg.zalo}</p>}
                                        </td>
                                        <td className="px-4 py-2.5">
                                            <p className="text-sm text-slate-600 dark:text-slate-300 max-w-[150px] line-clamp-2">{reg.course?.title ?? '—'}</p>
                                        </td>
                                        <td className="px-4 py-2.5 whitespace-nowrap">
                                            {schedule ? (
                                                <>
                                                    <p className="text-sm text-slate-600 dark:text-slate-300">{fmt(schedule.startDate)}</p>
                                                    {schedule.time && <p className="text-xs text-slate-400 mt-0.5">{schedule.time}</p>}
                                                </>
                                            ...
                                            <div className="flex items-center gap-1.5 overflow-visible">
                                                ...
                                                <div className="relative overflow-visible">
                                                    <DropdownSelect
                                                        ...
                                                        disabled={updatingId === reg.id}
                                                    />
                                                    {updatingId === reg.id && <Loader2 size={10} className="absolute right-1 top-1/2 -translate-y-1/2 text-red-500 animate-spin" />}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-4 py-2.5 text-sm text-slate-500 whitespace-nowrap">{fmt(reg.createdAt)}</td>
                                        <td className="px-4 py-2.5">
                                            <div className="flex items-center gap-1 justify-end">
                                                <button onClick={() => openDetail(reg)} title="Xem chi tiết" className="p-1.5 cursor-pointer text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors"><Eye size={15} /></button>
                                                <button onClick={() => openDelete(reg)}  title="Xóa đăng ký" className="p-1.5 cursor-pointer text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"><Trash2 size={15} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                );
                ...
                {meta && meta.totalPages > 1 && (
                    <div className="p-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between flex-wrap gap-3">
                        <p className="text-sm text-slate-500">
                            Trang <span className="font-semibold">{page}</span> / <span className="font-semibold">{meta.totalPages}</span> · Tổng <span className="font-semibold">{meta.total}</span> đăng ký
                        </p>
                        <div className="flex items-center gap-2 flex-wrap">
                            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} className="p-1.5 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 disabled:opacity-40 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"><ChevronLeft size={16} /></button>
                            {Array.from({ length: meta.totalPages }, (_, i) => i + 1)
                                ...
                                .map((item, i) => item === '…'
                                    ...
                                    : <button key={item} onClick={() => setPage(item)} className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${page === item ? 'bg-blue-600 text-white' : 'border border-slate-200 dark:border-slate-700 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800'}`}>{item}</button>
                                )}
                            <button onClick={() => setPage((p) => Math.min(meta.totalPages, p + 1))} disabled={page === meta.totalPages} className="p-1.5 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-500 disabled:opacity-40 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"><ChevronRight size={16} /></button>
                        </div>
                    </div>
                )}
            </div>

            {detailTarget && <DetailModal registration={detailTarget} onClose={closeDetail} onStatusChange={handleStatusChange} updatingId={updatingId} show={showDetailModal} duration={MODAL_DURATION} />}
            {deleteTarget && (
                <DeleteConfirmModal registration={deleteTarget}
                    onConfirm={async () => { await handleDeleteConfirm(); closeDelete(); }}
                    onCancel={closeDelete} loading={deleting} show={showDeleteModal} duration={MODAL_DURATION} />
            )}
        </div>
    );
};
