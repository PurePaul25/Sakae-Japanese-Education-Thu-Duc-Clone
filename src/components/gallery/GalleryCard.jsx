...
const GalleryCard = ({ image, handleLike, setSelectedImage }) => {
    ...
    return (
        ...
        >
            ...
            {/* Hover overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/25 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col justify-end p-3 md:p-4 rounded-xl">
                ...
                {/* Action row — like & comment on the same line */}
                <div className="flex items-center justify-between border-t border-white/10 pt-1.5 md:pt-3 transform translate-y-2 group-hover:translate-y-0 transition-transform duration-500 delay-75">
                    <div className="flex items-center gap-3">
                        {/* Like button — simple toggle, no reaction picker */}
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                handleLike(image.id, e, image.userReaction || 'LIKE');
                            }}
                            className="flex items-center gap-1 text-white hover:text-red-400 transition-colors focus:outline-none cursor-pointer"
                        >
                            <div className="flex -space-x-1.5 items-center">
                                {image.likesCount > 0 ? (
                                    topReactionEmojis.map((emoji, idx) => (
                                        <span key={idx} className="text-base drop-shadow-md">
                                            {emoji}
                                        </span>
                                    ))
                                ) : (
                            ...
                            {image.likesCount > 0 && (
                                <span className={`text-xs font-black ${image.isLiked ? 'text-red-400' : ''}`}>
                                    {image.likesCount}
                                </span>
                            )}
                        </button>
