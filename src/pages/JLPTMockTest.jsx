...
import { 
    FaClock, FaFlag, FaChevronLeft, FaChevronRight, 
    FaCheckCircle, FaFilter, FaBookOpen, FaTimes, 
    FaListAlt, FaInfoCircle, FaCheck, FaQuestionCircle,
    FaTrophy, FaRedo, FaHome
} from 'react-icons/fa';
...
const JLPTMockTest = () => {
    ...
    const allQuestions = currentTest ? currentTest.sections.reduce((acc, section) => {
            ...
            <div className="pt-28 pb-16 bg-gray-50 min-h-screen">
                <div className="max-w-6xl mx-auto px-4">
                    <div className="text-center mb-10">
                        <span className="inline-block px-4 py-1 bg-red-100 text-red-600 rounded-full font-bold mb-4 uppercase tracking-widest">Thi thử JLPT</span>
                        <h1 className="text-4xl md:text-5xl font-black text-gray-900 mb-6">Chọn Cấp Độ Thi Thử</h1>
                        <p className="text-gray-600 text-lg max-w-5xl mx-auto">Chọn cấp độ phù hợp để bắt đầu luyện đề. Mỗi đề thi đều được thiết kế sát với cấu trúc chuẩn JLPT mới nhất.</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
                        {levels.map((level) => (
                            ...
                            >
                                <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-all duration-300 group-hover:bg-opacity-80 ${level.id === 'N5' ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-600'}`}>
                    ...
                    <div className="mt-16 bg-white p-8 md:px-10 md:py-8 rounded-[2rem] shadow-sm border border-gray-100">
            ...
            <div className="pt-24 pb-12 bg-gray-50 min-h-screen">
                <div className="max-w-6xl mx-auto px-4">
                    ...
                    <h1 className="text-3xl font-black text-gray-900 mb-10 flex items-center gap-4">
                        Danh sách đề thi <span className="px-4 py-1 bg-red-600 text-white rounded-full text-lg">{selectedLevel}</span>
                    </h1>

                    {tests.length > 0 ? (
                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {tests.map(test => (
                                <div key={test.id} className="bg-white rounded-[2rem] overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 group">
                                    <div className="px-6 py-4">
                                        <div className="flex justify-between items-center mb-2">
                                            <div className="w-10 h-10 bg-gray-50 rounded-2xl flex items-center justify-center text-gray-400">
                                                <FaListAlt size={20} />
                                            </div>
                                            <span className="text-xs font-black text-gray-400 uppercase tracking-widest">{test.sections.length} PHẦN THI</span>
                                        </div>
                                        <h3 className="text-xl font-black text-gray-800 mb-2 group-hover:text-red-600 transition-colors">{test.title}</h3>
                                        <div className="flex items-center gap-6 text-sm text-gray-500 mb-6 font-medium">
                                            <span className="flex items-center gap-2"><FaClock className="text-red-500" /> {test.totalDuration} phút</span>
                                            <span className="flex items-center gap-2"><FaQuestionCircle className="text-orange-500" /> {test.sections.reduce((a, b) => a + b.questions.length, 0)} câu hỏi</span>
                                        </div>
                                        <button 
                        ...
                        <div className="bg-white p-20 rounded-[3rem] text-center shadow-sm">
                            ...
                            <p className="text-gray-500">Các đề thi {selectedLevel} đang được đội ngũ Sakae hoàn thiện.</p>
                        </div>
                    )}
                </div>
            </div>
    ...
    if (stage === 'TESTING') {
        ...
        return (
            <div className="bg-gray-50 min-h-screen pt-20">
                ...
                <div className="fixed top-17 left-0 right-0 bg-white/80 backdrop-blur-xl border-b border-gray-100 z-40 px-4 md:px-8 py-3 h-18 md:h-20 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="hidden sm:flex w-10 h-10 bg-red-600 text-white items-center justify-center rounded-xl font-black">
                            {currentTest.level}
                        </div>
                        <div className="font-black text-gray-800 truncate max-w-[150px] md:max-w-none">
                            {currentTest.title}
                        </div>
                    </div>

                    <div className="flex items-center gap-2 md:gap-4">
                        <div className={`flex items-center gap-2 px-3 md:px-5 py-1.5 rounded-2xl font-mono md:text-xl font-black transition-colors duration-300 ${timeLeft < 300 ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-900'}`}>
                ...
                <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 px-4 py-10 mt-14">
                    ...
                    <div className="flex-1 lg:max-w-3xl xl:max-w-4xl mx-auto w-full">
                        ...
                        {/* Question View */}
                        <div className="bg-white rounded-[2.5rem] p-6 md:py-6 shadow-sm border border-gray-100 mb-6">
                            <div className="flex justify-between items-center mb-4">
                                <span className="text-2xl font-black text-gray-400">Câu {currentQuestionIndex + 1}</span>
                                <button 
                                    onClick={() => setFlaggedQuestions(prev => ({ ...prev, [currentQuestion?.id]: !prev[currentQuestion?.id] }))}
                                    className={`p-2.5 rounded-xl cursor-pointer outline-none transition-all ${flaggedQuestions[currentQuestion?.id] ? 'bg-orange-100 text-orange-600' : 'bg-gray-50 text-gray-300 hover:text-orange-400'}`}
                            ...
                            <p className="text-lg md:text-xl text-gray-800 font-bold mb-6 leading-relaxed">
                            ...
                            <div className="grid gap-4">
                                {currentQuestion?.options.map((opt, idx) => (
                                    <button
                                        ...
                                        className={`group w-full p-3 rounded-3xl border-2 text-left transition-all flex items-center gap-3 ${
                                            answers[currentQuestion.id] === idx 
                                            ? 'border-red-600 bg-red-50/50 shadow-md ring-1 ring-red-600' 
                                            : 'border-gray-50 hover:border-red-200 bg-gray-50/30'
                                        }`}
                                    >
                                        ...
                                        <span className={`md:text-lg transition-all ${answers[currentQuestion.id] === idx ? 'text-gray-900 font-bold' : 'text-gray-600'}`}>
                                            {opt}
                                        </span>
                                    </button>
                                ))}
                    ...
                    <div className="lg:w-80 w-full shrink-0 mt-13">
                        <div className="lg:sticky lg:top-44 bg-white rounded-[2.5rem] p-6 shadow-sm border border-gray-100">
                                    ...
                                    <button 
                                        ...
                                        className={`relative h-11 w-full rounded-xl text-sm font-black transition-all flex items-center justify-center ${
                                            ...
                                            : answers[q.id] !== undefined
                                                ? 'bg-green-100 cursor-pointer text-green-700 hover:bg-green-200'
                            ...
                            <div className="mt-5 pt-5 border-t border-gray-100 space-y-4 text-xs font-bold uppercase tracking-wider">
                                ...
                                <div className="flex items-center gap-3 text-gray-400">
                                    <div className="w-4 h-4 rounded-md bg-green-100"></div> Đã trả lời
                                </div>
                                <div className="flex items-center gap-3 text-gray-400">
                                    <div className="w-4 h-4 rounded-md bg-orange-100"></div> Đã đánh dấu
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {showSubmitModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                        <div className="absolute inset-0 bg-gray-900/60 backdrop-blur-md" onClick={() => setShowSubmitModal(false)}></div>
                        <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full relative z-[110] text-center shadow-2xl animate-in zoom-in duration-300">
                            ...
                            <p className="text-gray-500 mb-8 text-lg">Bạn còn {allQuestions.length - Object.keys(answers).length} câu chưa trả lời. Thời gian vẫn còn {formatTime(timeLeft)} phút.</p>
        ...
        return (
            <div className="pt-28 pb-14 bg-gray-50 min-h-screen">
                <div className="max-w-7xl mx-auto px-4">
                    {/* Summary Card */}
                    <div className="bg-white rounded-[3rem] p-8 shadow-sm border border-gray-100 text-center mb-12 max-w-4xl mx-auto">
                        <div className="w-20 h-20 bg-yellow-100 text-yellow-600 rounded-[2rem] flex items-center justify-center mx-auto mb-6">
                            <FaTrophy size={40} />
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-3 md:gap-6 mb-6">
                            <div className="p-4 bg-red-50 rounded-[1.5rem]">
                                ...
                                <div className="text-[10px] md:text-xs font-black text-red-400 uppercase tracking-wider">Câu đúng</div>
                            </div>
                            <div className="p-4 bg-gray-50 rounded-[1.5rem]">
                                ...
                                <div className="text-[10px] md:text-xs font-black text-gray-400 uppercase tracking-wider">Tổng câu</div>
                            </div>
                            <div className="p-4 bg-green-50 rounded-[1.5rem]">
                                ...
                                <div className="text-[10px] md:text-xs font-black text-green-400 uppercase tracking-wider">Tỷ lệ</div>
                            </div>
                        </div>
                    ...
                    <div className="flex flex-col lg:flex-row gap-8">
                        ...
                        <div className="flex-1">
                            <div className="flex items-center justify-between mb-8">
                                ...
                                <button 
                                    ...
                                    className={`px-4 py-2.5 cursor-pointer rounded-xl font-bold transition-all text-sm md:text-base flex items-center gap-2 border-2 ${
                                        showAllAnswers 
                                    ...
                                    {allQuestions.map((q, idx) => (
                                        <div key={q.id} className="bg-white rounded-[2rem] p-8 shadow-sm border border-gray-100">
                                            <div className="flex items-center gap-3 mb-6">
                                                <span className={`px-4 py-1 rounded-lg font-black text-sm ${answers[q.id] === q.correct ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                                    Câu {idx + 1}
                                                </span>
                                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{q.sectionName}</span>
                                            </div>
                                            ...
                                            <div className="bg-blue-50/50 p-6 rounded-2xl border border-blue-100">
                                                <p className="text-blue-700 text-sm font-medium leading-relaxed">
                                                    <span className="font-black">Giải thích:</span> {q.explanation || 'Chưa có giải thích.'}
                                                </p>
                                            </div>
                                        </div>
                                    ))}
                                ...
                                <div className="transition-all duration-500">
                                    <div className="bg-white rounded-[2.5rem] p-6 shadow-sm border border-gray-100">
                                        <div className="flex items-center gap-4 mb-4">
                                            <span className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xl ${answers[reviewQuestion.id] === reviewQuestion.correct ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
                                                {reviewQuestionIndex + 1}
                                            </span>
                                            <div>
                                                <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-none mb-1">{reviewQuestion.sectionName}</div>
                                                <div className={`text-sm font-bold ${answers[reviewQuestion.id] === reviewQuestion.correct ? 'text-green-600' : 'text-red-600'}`}>
                                                    {answers[reviewQuestion.id] === reviewQuestion.correct ? 'Câu trả lời đúng' : 'Câu trả lời sai'}
                                                </div>
                                            </div>
                                        </div>

                                        <p className="text-lg md:text-xl text-gray-800 font-bold mb-6 leading-relaxed">{reviewQuestion.text}</p>
                                            ...
                                            {reviewQuestion.options.map((opt, oIdx) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        {isCorrect && <FaCheckCircle className="ml-auto text-green-600" size={24} />}
                                                        {!isCorrect && isUserChoice && <FaTimes className="ml-auto text-red-600" size={24} />}
                                                    </div>
                                                );
                                            })}
                                        </div>

                                        <div className="bg-blue-50/50 py-4 px-5 rounded-[2rem] border border-blue-100">
                                            <h4 className="flex items-center gap-2 text-blue-800 font-black mb-4">
                        ...
                        <div className="lg:w-80 w-full shrink-0">
                            <div className="lg:sticky lg:top-44 bg-white rounded-[2.5rem] py-5 px-6 shadow-sm border border-gray-100">
                                <h3 className="font-black text-gray-800 mb-3 flex items-center gap-3">
                                ...
                                <div className="mt-6 pt-5 border-t border-gray-100 space-y-4 text-xs font-bold uppercase tracking-wider">
                                    <div className="flex items-center gap-3 text-gray-400">
                                        <div className="w-4 h-4 rounded-md bg-blue-600"></div> Đang chọn
                                    </div>
                                    <div className="flex items-center gap-3 text-gray-400">
                                        <div className="w-4 h-4 rounded-md bg-green-100"></div> Đã đúng
                                    </div>
                                    <div className="flex items-center gap-3 text-gray-400">
                                        <div className="w-4 h-4 rounded-md bg-red-100"></div> Đã sai
                                    </div>
                                </div>
                            </div>
                        </div>
